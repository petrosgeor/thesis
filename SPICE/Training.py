...
def ExpMaxAlgorithmTraining1(train_dataloader: CustomDataloader, eval_loader: CustomDataloader, num_epochs: int, M: int, K: int) -> Network:
    try:
    ...
    for epoch in range(0, num_epochs):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            #kl_loss = kl_divergance(X_strong_predictions[known_indices, :], X_strong_predictions[unknown_indices, :])
            #kl_loss = kl_divergance(X_strong_predictions[known_indices,:], X_strong_predictions[unknown_indices,:])
            kl_loss = ClusterLoss(X_strong_predictions)

            #loss = supervised_loss + unsupervised_loss - kl_loss * 10**(-3)
            loss = supervised_loss + unsupervised_loss + kl_loss
...
ExpMaxAlgorithmTraining1(train_dataloader=train_dataloader, eval_loader=eval_loader, num_epochs=2, M=300, K=20)
