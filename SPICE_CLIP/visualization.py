...
def VisualizeWithTSNE(resnet_embeddings: np.ndarray, labels: np.ndarray) -> None:
    ...
    X_embedded = TSNE(n_components=2, perplexity=10).fit_transform(resnet_embeddings)
    ...
    for i, color in zip(unique_labels, colors):
        plt.scatter(X_embedded[labels == i, 0], X_embedded[labels == i, 1], c=[color], label=f'Label {i}', alpha=0.6)
...
weak_aug = Weak_Augmentation()
strong_aug = Strong_Augmentation()
dataset = CIFAR100(identity_transform=identity_aug, weak_transform=weak_aug, strong_transform=strong_aug, pretrain=True)
...
if dataset.pretrain == True:
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader):
            ...
            batch_embeddings = resnet(X_batch)
            embeddings.append(batch_embeddings.cpu())
            labels.append(batch_Ids)
